...
if __name__ == "__main__":
    ...
    directories = sorted([d for d in os.listdir(root_folder_path) if os.path.isdir(os.path.join(root_folder_path, d))])
    ...
    with open(output_file_path, 'w', encoding='utf-8') as output_file:
        for directory in directories:
            ...
            for root, dirs, files in os.walk(folder_path):
                for file in files:
                    if file.endswith(('.xci', '.nsp')) and all(keyword not in file for keyword in ['(UPD)', '(DLC)']):                  
